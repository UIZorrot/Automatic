...
- `npx @txzy/automatic-bridge smoke`
- `npx @txzy/automatic-bridge once`
...
- `AUTOMATIC_BASE_URL` defaults to `https://tool.auto.txzy.net` for the public deployment (override only if you are pointing at a different environment)
...
- `npx @txzy/automatic-bridge doctor` completes and reports a reachable setup
- `npx @txzy/automatic-bridge smoke` completes without crashing
